...
- **排程**：APScheduler
- **開發期對外連線**：ngrok
- **管理員 Dashboard**：Jinja2 templates + 靜態檔（原生 CSS / Vanilla JS），無前端框架、無建置流程
...
└── app/
    ...
    ├── main.py            FastAPI 主程式、Webhook、所有 API 路由、排程任務
    ├── templates/         管理員 Dashboard 的 Jinja2 樣板
    │   ├── _base.html         共用外框（head、樣式引入）
    │   ├── _nav.html          四個頁面共用的導覽列
    │   ├── dashboard.html     /admin 主控台
    │   ├── reports.html       /admin/reports 每日報表
    │   ├── exceptions.html    /admin/exceptions 例外處理
    │   └── residents.html     /admin/residents 住戶綁定管理
    └── static/
        ├── css/
        │   ├── base.css       共用樣式（版面、按鈕、表格）
        │   └── dashboard.css  主控台專用樣式
        └── js/
            ├── common.js      共用工具函式
            ├── dashboard.js   主控台
            ├── reports.js     每日報表
...
| 排程 | 週期 | 用途 |
|---|---|---|
| `check_pickup_timeout` | 1 分鐘 | `arrived` 超過 8 分鐘住戶都沒動作，轉成 `returned_timeout` 自動退回。送貨、退貨共用這一支（只看 `status`，不分 `task_type`） |
| `check_schedule_reminder` | 1 分鐘 | 跨日預約取貨的包裹，在時段開始前 2 小時提醒收件人一次，寫入 `schedule_reminder_sent_at`，不改變狀態 |
| `check_assign_timeout` | 1 分鐘 | 艙門分配了但管理員一直沒裝載，逾時釋放艙門 |
| `check_return_timeout` | 1 分鐘 | 退回流程中，管理員按「開門」後超過 8 分鐘沒按「關門」，請機器人自動關門並清空 `return_door_opened_at`，讓管理員可以重新開門 |
